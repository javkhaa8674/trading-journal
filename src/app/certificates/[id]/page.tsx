    ...
    const loadData = async () => {
      try {
        ...
        setFormData({
          ...
          prop_firm_name: certificate.prop_firm_name || "",
          certificate_type: certificate.certificate_type || "standard",
          challenge_level: certificate.challenge_level || "phase_1",
          certificate_url: certificate.certificate_url || "",
        });

        if (certificate.certificate_url) {
          setExistingImage(certificate.certificate_url);
        }
      } catch (error) {
  ...
  const deleteOldImage = async (oldUrl: string) => {
    try {
      ...
      const filePath = urlParts
        .slice(urlParts.indexOf("certificates"))
        .join("/");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (uploadedUrl) {
          if (existingImage) {
            await deleteOldImage(existingImage);
          }
      ...
      alert("Certificate шинэчлэхэд алдаа гарлаа.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-lg border bg-white p-6 shadow-sm dark:bg-gray-900">
          ...
          <div className="space-y-4">
            ...
            {/* Image Upload - Fixed aspect ratio warning */}
            <div>
              <label className="block text-sm font-medium mb-1">
                Certificate зураг
              </label>
